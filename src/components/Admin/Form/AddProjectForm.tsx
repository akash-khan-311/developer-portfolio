    ...
    <form onSubmit={handleSubmit(onSubmit)} action="">
      <div>
        ...
        >
          <input
            ...
            accept="image/*"
            onChange={loadFile}
            className="w-full px-3 py-3 border bg-slate-900 border-gray-300 rounded-md focus:outline-none focus:border-blue-500"
          />
        </Field>
      </div>
      <div>
      ...
      <div>
        ...
        <ul className="mt-2 space-y-1 inline">
          {tech?.map((text, index) => (
            <li key={index} className=" flex items-center px-3 py-1 rounded-md">
      ...
      <div className="flex justify-between items-center mt-6">
        <button
          type="submit"
          className="w-full px-8 py-2 mr-1 bg-green-600 font-sans text-xs font-bold text-white uppercase transition-all rounded-lg "
        >
