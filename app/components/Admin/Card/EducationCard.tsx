  ...
  return (
    <div className="border rounded-lg p-5 relative">
      <div className="flex flex-col gap-y-5">
        <div>( <span>{formatDate(data?.passYear)}</span> - <span>{formatDate(data?.admitYear)}</span> )</div>
        ...
        <EducationModal mutate={mutate} data={data} modalOpen={modalOpen} setModalOpen={setModalOpen} />
      </div>
    </div>
  )
}
